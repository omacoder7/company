...
<form action="{{ route('admin.developer-tasks.store') }}" method="POST" style="max-width: 1200px;">
    @csrf
    ...
    <div style="margin-top: var(--spacing-xl); padding-top: var(--spacing-lg); border-top: 2px solid var(--color-border);">
        <div class="form-group">
            ...
            <input type="text" name="stack" class="form-input" value="{{ old('stack') }}" placeholder="PHP, Laravel, Vue.js...">
            @error('stack')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="format" class="form-input" value="{{ old('format') }}" placeholder="Удаленно, полный день...">
            @error('format')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>

...
<form action="{{ route('admin.services.update', $service->id) }}" method="POST" style="max-width: 1200px;">
    @csrf
    @method('PUT')
    ...
    @php
        $translation = $translations[$locale] ?? ['title' => '', 'description' => '', 'problem' => '', 'solution' => '', 'result' => ''];
    @endphp
    <div class="language-content" id="lang-{{ $locale }}" style="display: {{ $locale === 'ru' ? 'block' : 'none' }};">
        ...
        <div class="form-group">
            ...
            <textarea name="translations[{{ $locale }}][problem]" class="form-textarea" rows="4">{{ old("translations.$locale.problem", $translation['problem']) }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea name="translations[{{ $locale }}][solution]" class="form-textarea" rows="4">{{ old("translations.$locale.solution", $translation['solution']) }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea name="translations[{{ $locale }}][result]" class="form-textarea" rows="4">{{ old("translations.$locale.result", $translation['result']) }}</textarea>
    ...
    <div style="margin-top: var(--spacing-xl); padding-top: var(--spacing-lg); border-top: 2px solid var(--color-border);">
        <div class="form-group">
            ...
            <input type="number" name="order" class="form-input" value="{{ old('order', $service->order) }}" min="1">
            @error('order')
                <div class="error-message">{{ $message }}</div>
            @enderror
        </div>
